...
class AuthBloc extends DriftedBloc<AuthEvent, AuthState> {
  ...
  late AuthRepository authRepository = AuthRepository(firebaseService: firebaseService);
  late UserRepository userRepository = UserRepository(firebaseService: firebaseService);
      ...
      if(state.status == AuthStatus.authenticated){
        ...
        if(token != null && token.isNotEmpty){
          ...
          if(currentToken != token){
           ...
           await secureStorageRepository.write.firebaseMessagingToken(token);
          }
        }
      }
    },
